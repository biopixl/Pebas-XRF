...
Visual examination of integrated core images (Figures~\ref{fig:group1}--\ref{fig:group7}) confirms correspondence between geochemical signals and lithological features:

\begin{itemize}
    \item Shell-rich intervals (Ca $>$ 100~kcps) correspond to visible shell concentrations in optical images
    \item Facies transitions align with lithological boundaries
    \item Darker, clay-rich intervals correspond to lower Ca and higher Ti signals
\end{itemize}

This optical-geochemical correspondence validates the facies classification scheme and demonstrates that XRF signals capture genuine sedimentological variations rather than analytical artifacts.
...
Complementary CT scanning provides an independent physical validation of XRF-derived carbonate estimates through bulk density measurement (Figure~\ref{fig:ct_integration}).

\subsubsection{Quantitative Correlation}

Systematic alignment optimization between CT density profiles (80 slices, 2~mm resolution) and XRF data identified TAM-1-2-3B-A as the best-matching section with:

\begin{itemize}
    \item \textbf{Correlation coefficient:} $r$ = 0.877 ($p$ = 0.022)
    \item \textbf{Optimal depth offset:} 24~mm
    \item \textbf{Overlap coverage:} 58~mm of valid core material
\end{itemize}

This strong positive correlation confirms that XRF Ca (surface geochemistry) and CT density (bulk physical property) track the same underlying signal: carbonate mineral content.

\subsubsection{Physical Interpretation}

The CT-Ca correlation arises from fundamental material properties:
...
\begin{itemize}
    ...
    \item Higher carbonate content $\rightarrow$ higher bulk density $\rightarrow$ greater CT attenuation
...
The CT-XRF integration demonstrates that:
...
\caption{CT-XRF integration analysis validating multi-proxy approach. (A) CT density profile from SE1 core region. (B) XRF Ca profile from TAM-1-2-3B-A after depth alignment (offset = 24~mm). (C) Scatter plot showing strong positive correlation ($r$ = 0.877, $p$ = 0.022). (D) Aligned profiles demonstrating correspondence between bulk density and carbonate content. This independent validation confirms that XRF surface geochemistry accurately tracks bulk sediment composition.}
...
\begin{enumerate}
    \item \textbf{Multi-proxy validation:} Independent convergence of XRF surface geochemistry and CT bulk density measurements ($r$ = 0.877, $p$ = 0.022) validates the reliability of non-destructive core scanning methods for Pebas Formation sediments. This correlation confirms that XRF-derived Ca faithfully tracks bulk carbonate content, not surface artifacts.
