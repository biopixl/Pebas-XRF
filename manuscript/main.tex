...
\begin{itemize}
    ...
    \item \textbf{Density-carbonate correlation:} CT attenuation values correlate strongly with XRF Ca signal ($r$ = 0.877, $p$ = 0.022), as carbonate minerals (aragonite, calcite) have higher X-ray attenuation than siliciclastic mud. This provides independent validation of carbonate-rich horizons identified by XRF.

    \item \textbf{Best-matching section:} Systematic correlation analysis across all XRF sections identified TAM-1-2-3B-A as the best match for the CT scan, with an optimal depth offset of 24~mm. The strong positive correlation confirms that CT density increases with carbonate content.

    \item \textbf{Structural features:} CT imaging reveals sedimentary structures, shell concentrations, and density variations (Figure~\ref{fig:ct_integration}). The CT core region spans approximately 58~mm of valid core material within the SE1 (TOP) series.

    \item \textbf{Depth alignment:} DICOM SliceLocation metadata enables precise depth registration between CT slices and XRF measurement positions (3~mm step size). Offset optimization analysis achieved alignment with $r > 0.85$ correlation.
\end{itemize}

The integrated CT-XRF analysis demonstrates that bulk density (CT) and surface geochemistry (XRF) provide consistent, complementary information about carbonate content in Pebas Formation sediments.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/ct_xrf_final_alignment.png}
\caption{CT-XRF integration analysis. (A) CT density profile from SE1 core region showing depth-dependent variations. (B) XRF Ca profile from TAM-1-2-3B-A after depth alignment (offset = 24~mm). (C) Scatter plot showing strong positive correlation between CT density and XRF Ca ($r$ = 0.877, $p$ = 0.022). (D) Aligned CT and XRF profiles demonstrating correspondence between bulk density and carbonate content. Higher Ca (more carbonate) corresponds to less negative CT values (higher density).}
...
\begin{enumerate}
    ...
    \item \textbf{CT-XRF integration:} Complementary CT scanning data (80 slices across two series) shows strong correlation with XRF Ca signal ($r$ = 0.877), validating carbonate content estimates and demonstrating that bulk density (CT) and surface geochemistry (XRF) provide consistent paleoenvironmental information.
\end{enumerate}
