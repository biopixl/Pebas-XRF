...
In the Pebas context, baseline Zr/Rb values are consistently low (median $<$ 1.3 at both sites), indicating predominantly fine-grained, low-energy lacustrine deposition. This is consistent with the Pebas mega-wetland model of shallow lakes and embayments \citep{Wesselingh2006}. Discrete Zr/Rb spikes ($z$-score $>$ 2 relative to site mean) identify high-energy events---potentially floods, storm layers, or deltaic progradation pulses. Our analysis detected 20 such events at TAM (concentrated in GROUP2--3) and 14 at SC (concentrated in GROUP4), suggesting episodic hydrodynamic disturbance of the otherwise quiescent depositional environment.
...
\subsection{Proxy Limitations: Sr/Ba Paleosalinity}
\label{subsec:srba_limitations}

Several commonly-used XRF proxies were evaluated but not adopted for this study due to calibration limitations in the Pebas Formation context.

\paragraph{Sr/Ba Paleosalinity Proxy.}
The Sr/Ba ratio has been proposed as a paleosalinity indicator, with published thresholds of $<$0.2 (freshwater), 0.2--0.5 (brackish), and $>$0.5 (marine) based on calibration with modern sediments \citep{Wei2020}. However, these thresholds were developed for fine-grained siliciclastic sediments \emph{without significant carbonate content}, where elements are acquired through adsorption of dissolved species.

In the Pebas Formation, carbonate-bearing sediments show dramatically elevated Sr/Ba values (median = 18--19 at both sites), exceeding the ``marine'' threshold by a factor of 35--40$\times$. This reflects Sr incorporation into carbonate phases (especially aragonite shells) rather than salinity. The strong Ca--Sr correlation ($r$ = 0.62) confirms that carbonate mineralogy, not watermass salinity, controls Sr distribution in this setting.

Independent Sr isotope studies on Pebas molluscs indicate predominantly freshwater conditions ($^{87}$Sr/$^{86}$Sr = 0.7081--0.7085), with rare marine incursions reaching maximum $\sim$5 psu \citep{Vonhof2003, Boonstra2021}. The XRF Sr/Ba proxy is therefore \emph{not recommended} for paleosalinity reconstruction in carbonate-bearing Pebas sediments.

\paragraph{Cu/Zn Alternative Redox Proxy.}
The Cu/Zn ratio has been proposed as an alternative redox proxy, with lower values indicating more reducing conditions \citep{Tribovillard2006}. However, in the Pebas dataset, Cu/Zn shows a positive correlation with Fe/Mn ($r$ = 0.24) when a negative correlation would be expected if both tracked the same redox signal. This inconsistency suggests that Cu/Zn behavior in the Pebas setting is influenced by factors beyond simple redox chemistry, possibly including organic matter complexation or sulfide precipitation dynamics. We retain Fe/Mn as the primary redox indicator given its established mechanistic basis and literature support in lacustrine settings.
